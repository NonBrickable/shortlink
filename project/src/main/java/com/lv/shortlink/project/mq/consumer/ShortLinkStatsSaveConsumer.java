...
import com.lv.shortlink.project.common.constant.RedisKeyConstant;
...
import com.lv.shortlink.project.mq.idempotent.MessageQueueIdempotentHandler;
import com.lv.shortlink.project.mq.producer.DelayShortLinkStatsProducer;
...
/**
 * 短链接监控状态保存消息队列消费者（已完成）
 */
@Slf4j
@Component
@RequiredArgsConstructor
@RocketMQMessageListener(
        topic = RocketMQConstant.STATUS_SAVE_TOPIC,
        consumerGroup = RocketMQConstant.STATUS_COMSUMER_GROUP
)
public class ShortLinkStatsSaveConsumer implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt msg) {
        ...
        try {
            Map<String, String> producerMap = JSONObject.parseObject(msg.getBody().toString(), Map.class);
            String fullShortUrl = producerMap.get("fullShortUrl");
            if (StrUtil.isNotBlank(fullShortUrl)) {
                String gid = producerMap.get("gid");
                ShortLinkStatsRecordDTO statsRecord = JSON.parseObject(producerMap.get("statsRecord"), ShortLinkStatsRecordDTO.class);
                actualSaveShortLinkStats(fullShortUrl, gid, statsRecord, producerMap);
            }
        }catch (Throwable ex){
            // 某某某情况宕机了
    ...
    public void actualSaveShortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord,Map<String, String> producerMap) {
